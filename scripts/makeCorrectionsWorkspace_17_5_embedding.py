...
######################################
### KIT muon tag and probe results ###
######################################
...
for task in SF_histsToWrap:
    wsptools.SafeWrapHist(
        w, ['m_pt', 'expr::m_abs_eta("TMath::Abs(@0)",m_eta[0])'],
        GetFromTFile(task[0]),
        name=task[1])
wsptools.MakeBinnedCategoryFuncMap(
    w, 'm_iso', [0., 0.15, 0.25, 0.50], 'm_iso_binned_data',
    ['m_iso_data', 'm_aiso1_data', 'm_aiso2_data'])
wsptools.MakeBinnedCategoryFuncMap(w, 'm_iso', [0., 0.15, 0.25, 0.50],
                                   'm_iso_binned_mc',
                                   ['m_iso_mc', 'm_aiso1_mc', 'm_aiso2_mc'])
wsptools.MakeBinnedCategoryFuncMap(
    w, 'm_iso', [0., 0.10, 0.20, 0.50], 'm_iso_binned_data',
    ['m_iso_data', 'm_aiso1_data', 'm_aiso2_data'])
...
wsptools.MakeBinnedCategoryFuncMap(
    w, 'm_iso', [0., 0.10, 0.20, 0.50], 'm_trg_binned_data',
    ['m_trg_data', 'm_trg_aiso1_data', 'm_trg_aiso2_data'])
wsptools.MakeBinnedCategoryFuncMap(
    w, 'm_iso', [0., 0.10, 0.20, 0.50], 'm_trg_binned_mc',
    ['m_trg_mc', 'm_trg_aiso1_mc', 'm_trg_aiso2_mc'])
# '''
# for t in ['sel_idEmb','sel_vvliso']:
#     w.factory('expr::m_%s_ratio("(1.0)/@0", m_%s_data)' % (t, t))
# '''
for t in ['id', 'iso', 'trg', 'aiso1', 'aiso2', 'looseiso']:
    w.factory('expr::m_%s_ratio("min(1.99,(@0/@1))", m_%s_data, m_%s_mc)' %
              (t, t, t))
...
SF_histsToWrap = [
    ...
    (loc + '/ZeeTP_Embedding_Fits_ID90_pt_eta_bins.root:ID90_pt_eta_bins',                         'e_id90_mc'),
    (loc + '/ZeeTP_Data_Fits_ID80_pt_eta_bins.root:ID80_pt_eta_bins',                              'e_id80_data'),
    (loc + '/ZeeTP_Embedding_Fits_ID80_pt_eta_bins.root:ID80_pt_eta_bins',                         'e_id80_mc'),
...
for task in SF_histsToWrap:
    wsptools.SafeWrapHist(
        w, ['e_pt', 'expr::e_abs_eta("TMath::Abs(@0)",e_eta[0])'],
        GetFromTFile(task[0]),
        name=task[1])
wsptools.MakeBinnedCategoryFuncMap(
    w, 'e_iso', [0., 0.10, 0.20, 0.50], 'e_iso_binned_data',
    ['e_iso_data', 'e_aiso1_data', 'e_aiso2_data'])
wsptools.MakeBinnedCategoryFuncMap(w, 'e_iso', [0., 0.10, 0.20, 0.50],
                                   'e_iso_binned_mc',
                                   ['e_iso_mc', 'e_aiso1_mc', 'e_aiso2_mc'])
...
wsptools.MakeBinnedCategoryFuncMap(
    w, 'e_iso', [0., 0.10, 0.20, 0.50], 'e_trg_binned_data',
    ['e_trg_data', 'e_trg_aiso1_data', 'e_trg_aiso2_data'])
wsptools.MakeBinnedCategoryFuncMap(
    w, 'e_iso', [0., 0.10, 0.20, 0.50], 'e_trg_binned_mc',
    ['e_trg_mc', 'e_trg_aiso1_mc', 'e_trg_aiso2_mc'])

for t in [
        'id80', 'id90', 'iso', 'trg', 'aiso1', 'aiso2', 'trg_binned', 'iso_binned',
        'looseiso'
]:
    w.factory('expr::e_%s_ratio("min(1.99,(@0/@1))", e_%s_data, e_%s_mc)' %
              (t, t, t))

for t in ['id80', 'id90', 'iso', 'trg', 'aiso1', 'aiso2', 'looseiso']:
...
with open('fitresults_tt_moriond2017.json') as jsonfile:
    ...
    for tautype in ['genuine', 'fake']:
        for iso in [
                'VLooseIso', 'LooseIso', 'MediumIso', 'TightIso', 'VTightIso',
                'VVTightIso'
        ]:
            for dm in ['dm0', 'dm1', 'dm10']:
                ...
                w.factory(
                    'CrystalBallEfficiency::t_%s_tt_data(t_pt[0],%g,%g,%g,%g,%g)'
                    % (label, x['m_{0}'], x['sigma'], x['alpha'], x['n'],
                       x['norm']))

                x = pars['mc_%s' % (label)]
                w.factory(
                    'CrystalBallEfficiency::t_%s_tt_mc(t_pt[0],%g,%g,%g,%g,%g)'
                    % (label, x['m_{0}'], x['sigma'], x['alpha'], x['n'],
                       x['norm']))
            label = '%s_%s' % (tautype, iso)
            wsptools.MakeBinnedCategoryFuncMap(
                w, 't_dm', [-0.5, 0.5, 9.5, 10.5], 't_%s_tt_data' % label, [
                    't_%s_dm0_tt_data' % label,
                    't_%s_dm1_tt_data' % label,
                    't_%s_dm10_tt_data' % label
                ])
            wsptools.MakeBinnedCategoryFuncMap(
                w, 't_dm', [-0.5, 0.5, 9.5, 10.5], 't_%s_tt_mc' % label, [
                    't_%s_dm0_tt_mc' % label,
                    't_%s_dm1_tt_mc' % label,
                    't_%s_dm10_tt_mc' % label
                ])
            w.factory(
                'expr::t_%s_tt_ratio("@0/@1", t_%s_tt_data, t_%s_tt_mc)' %
                (label, label, label))
...
w.Print()
w.writeToFile('htt_scalefactors_v17_5_embedded.root')
w.Delete()
